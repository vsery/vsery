<div class="list-group" data-spy="affix">
    <volist name="sidebar" id="option" key="sort">
        ...
        <!-- <if condition="($option[ 'text'] eq '首页') OR ($option[ 'text'] eq '菜单')">
            ...
            <else />
            <li class="dropdown">
                <a class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" title="{$option.text}"> <i class="fa fa-fw {$option.icon}"></i> {$option.text} <span class="caret"></span> </a>
                <ul class="dropdown-menu">
                    <li><a href="#" class="">{$option.text} - 列表</a></li>
                    <li role="separator" class="divider"></li>
                    <li><a href="#" class="">{$option.text} - 回收站</a></li>
                </ul>
            </li>
        </if> -->
    </volist>
